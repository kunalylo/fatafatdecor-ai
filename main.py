...
DETECT_SYSTEM_PROMPT = (
    ...
    "VENUE vs DECORATION (do not bill what was already there):\n"
    "• Only list items the decorator BRINGS AND INSTALLS. The venue's own pool, deck,\n"
    "  pergola, garden plants, hedges, built-in lighting, sofas and dining furniture\n"
    "  are NOT decoration items — skip them entirely.\n"
    "• Include furniture/plants ONLY when clearly styled in as part of the setup\n"
    "  (e.g. a plinth holding the cake, a rented bar cart, potted palms flanking a\n"
    "  backdrop). If in doubt whether it belongs to the venue, leave it out.\n\n"
    "TEXT ACCURACY (never guess):\n"
    "• Fill text_content ONLY when you can clearly READ the characters. If the text is\n"
    "  blurry, tiny, angled, reflected or in a script you cannot read, use an empty\n"
    "  string \"\" — NEVER invent or approximate words. A wrong sign text ruins the event.\n\n"
    ...
    "• FILLERS ARE MANDATORY: every organic garland is packed with small filler\n"
    "  balloons wedged in the gaps BETWEEN the big balloons — they are what makes\n"
    "  the garland look full. Look INTO the gaps and along the garland's tail.\n"
    "  OUR STANDARD FILLER IS 8\" — report garland fillers as 8\" by default, and\n"
    "  use 5\" ONLY when you clearly see a second, distinctly tinier accent tier\n"
    "  alongside the 8\" fillers. If your garland list has no 8\" row, you have\n"
    "  missed the fillers — go back and count again. Typically 30-40%% of a\n"
    "  garland's balloons are fillers.\n\n"
    "OTHER LOOK-ALIKES:\n"
    "• THE THREE WALLS — these are different products at very different costs; look\n"
    "  closely at the SURFACE before choosing:\n"
    "  – Thousands of small SQUARE/ROUND SHINY DISCS in a fine grid that catch light\n"
    "    individually (a sequin/shimmer wall, often flips colour, sparkles) →\n"
    "    structure/shimmer_wall. Count the ~2ft-wide panels as quantity.\n"
    "  – A grid of BIG PUFFY AIR-FILLED QUILTED CELLS with rounded edges and weld\n"
    "    seams (an inflatable pillow wall) → structure/panel_wall.\n"
    "  – LIMP VERTICAL STRIPS with gaps you can see through (a foil fringe curtain)\n"
    "    → backdrop/foil_curtain.\n"
    "  A shimmer wall is NOT a pillow wall: discs are small, flat and numerous; pillow\n"
    "  cells are large, inflated and few. Say which one you saw in the description.\n"
    ...
    "QUANTITY UNITS — est_unit_cost_inr must be the price of ONE unit of `quantity`:\n"
    "• A BANNER/GARLAND SET sold as one piece (a strung 'Happy Birthday' letter banner,\n"
    "  a light-up letter string, a bunting) = quantity 1 (ONE set), text_content = the\n"
    "  words, est_unit_cost_inr = the price of the whole set (~Rs 400). Do NOT set\n"
    "  quantity to the number of letters — that multiplies the set price by 13.\n"
    "• Individual large foil LETTER/NUMBER BALLOONS (each 16\"+ mylar, inflated\n"
    "  separately) = quantity = how many balloons, est_unit_cost_inr = per balloon\n"
    "  (~Rs 120).\n"
    "If unsure, ask: is this ONE product off a shelf, or several separate products?\n\n"
